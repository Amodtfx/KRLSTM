...
# load data
load_data = read_csv('data/csv/test.csv', header=0)

# load model
model = load_model("models/test_2017_retrained_0.h5")
...
for i in range(len(forecasts)):
    pred_1.append(forecasts[i][0])  
    pred_2.append(forecasts[i][1])
    pred_3.append(forecasts[i][2])  
    actual_1.append(actual[i][0])
    actual_2.append(actual[i][1])
    actual_3.append(actual[i][2])
#----------------------------------------- 
for i in range(len(forecasts)):
...
print('RMSE TRAIN +1: {:.2f}; RMSE TRAIN +2 : {:.2f}; RMSE TRAIN +3 : {:.2f} '.format(rmse_plus_train_1, rmse_plus_train_2, rmse_plus_train_3)) 
print('RMSE TEST +1:  {:.2f}; RMSE TEST +2  : {:.2f}; RMSE TEST +3  : {:.2f} '.format(rmse_plus_test_1, rmse_plus_test_2, rmse_plus_test_3)) 

# plot forecasts
plot_forecasts(price_series, forecasts)