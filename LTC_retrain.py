...
for i in range(1,307):
    ...
    # load dataset
    load_data_window = load_data[:239+j+n_seq]
    # load model
    model = load_model("models/LTC/test_2017_retrained_%d.h5" % (i-1))
    ...
    model.save("models/LTC/test_2017_retrained_%d.h5" % i, overwrite=True)
    del model